...
namespace Crm.Projects
{
        ...
        : EfCoreRepository<CrmDbContext, Project, Guid>(dbContextProvider), IProjectRepository
    {
        ...
        {
            var query = ApplyDataFilters((await GetQueryableAsync()).AsNoTracking(), name, description, startTime, endTime, statues, revenue, succesRate, employeeIds, customerId);
        ...
        {
            query=query.WhereIf(!string.IsNullOrWhiteSpace(name), e => e.Name == name)
