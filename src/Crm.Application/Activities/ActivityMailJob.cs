...
namespace Crm.Activities
{
    public class ActivityMailJob(IActivityRepository activityRepository,
        ...
        IStringEncryptionService stringEncryptionService) :  ITransientDependency
    {
    ...
    public async Task SendActivityNotificationAsync(Guid activityId)
    {
        ...
        var user = await userRepository.GetAsync(employee.UserId);
            ...
            //Console.WriteLine($"Encrypted value: {stringEncryptionService.Encrypt("virfuzfdcrihtprn")}");
            Console.WriteLine($"şifre:{Volo.Abp.Emailing.EmailSettingNames.Smtp.Password}");
        ...
        await emailSender.SendAsync(
           ...
           //var result = settingEncryptionService.Encrypt(new SettingDefinition("Abp.Mailing.Smtp.Password") ,"virfuzfdcrihtprn");
